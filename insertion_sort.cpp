...
int main() {
	/*Declaring all the arrays which will be testing the timing of Insertion sort*/
    int n1, n2, n3, n4;
    n1 = 10000;
    n2 = 20000;
    n3 = 50000;
...
void insertion_sort (int* a, int array_size){
    ...
    for(i = 1; i < array_size; i++){ //outer for loop running through entire array
    	min_value = a[i];
        for(j = i-1; j >=0; j--){ //inner loop running from i-1 to 0
            if(a[j] > min_value){ //if the value is less than min
                a[j+1] = a[j]; //shifting
            }
        }
        a[j+1] = min_value;

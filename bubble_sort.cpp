...
int main() {
	/*Declaring all the arrays which will be testing the timing of Bubble sort*/
    int n1, n2, n3, n4;
    n1 = 10000;
    n2 = 20000;
    n3 = 50000;
...
void bubble_sort (int* a, int array_size){
    int i,j;
    for(i = array_size; i > 0; i--){ //outer loop
        for(j = 0; j < i-1; j++){ //inner loop with decraseing interation size with every iteration of outer loop
            if(a[j] > a[j+1]){
                swap(&(a[j]), &(a[j+1]));
            }
        }
